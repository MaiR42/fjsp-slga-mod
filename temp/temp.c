...
/* ################################################################
 * ##  NUEVO: instrumentacion de repair_chromosome                ##
 * ##  Agregada para medir la hipotesis de que el cruce de MS     ##
 * ##  independiente del OS destruia genes heredados al reparar.  ##
 * ##  Resultado medido: ~40% de destruccion en Brandimarte antes ##
 * ##  del fix de pox_build_child (ver mas abajo); 0% despues.    ##
 * ################################################################ */
...
/* <-- NUEVO: torneo de tamano k generico */
...
/* ################################################################
 * ##  MODIFICADO: pox_build_child ahora hereda MS ACOPLADO al   ##
 * ##  mismo mecanismo de POX, en vez de un cruce de MS "dos     ##
 * ##  puntos" independiente del OS. Esto elimina casi toda la   ##
 * ##  perdida de informacion genetica que causaba repair()      ##
 * ##  (medido: ~40% de genes MS destruidos -> 0% con este fix). ##
 * ##  Ver commit correspondiente para el diagnostico completo.  ##
 * ################################################################ */
/*
 ...
 *
 * MEJORA respecto a la version anterior: el MS ahora se hereda ACOPLADO
 * al mismo mecanismo de POX, en vez de cruzarse por separado con "dos
 * puntos" independientes del OS:
 *   - Posiciones conservadas: MS[i] = keep_parent->MS[i] (misma posicion,
 *     garantizado valido porque nunca cambia de indice).
 *   - Posiciones rellenadas: MS[i] = fill_parent->MS[idx], donde idx es
 *     la posicion EXACTA de fill_parent de donde se tomo esa operacion
 *     especifica (garantizado valido porque esa fue su maquina real en
 *     el padre de origen).
 * Con esto, el hijo NUNCA hereda una combinacion (operacion, maquina)
 * invalida -> repair_chromosome pasa a ser solo una red de seguridad,
 * en vez de destruir en promedio ~40% de los genes MS heredados (medido
 * empiricamente antes de este cambio, ver diagnostico).
 */
...
void crossover(const FJSPInstance *inst, const Chromosome *p1, const Chromosome *p2,
               Chromosome *c1, Chromosome *c2)
{
    ...
    /* <-- MODIFICADO: antes habia aca un cruce de MS "dos puntos" por
     * separado (mismo bloque, ~15 lineas); se elimino porque pox_build_child
     * ahora hereda MS acoplado al OS (ver banner arriba de esa funcion). */
     ...
     * al POX, cada gen heredado ya es valido por construccion. Se deja el
     * llamado igual para cubrir cualquier caso borde no contemplado. */
...
void repair_chromosome(const FJSPInstance *inst, Chromosome *c)
{
    for (int i = 0; i < c->length; i++) {
        ...
        g_repair_total_checked++;   /* <-- NUEVO: instrumentacion */
        if (!valid) {
            g_repair_total_repaired++;   /* <-- NUEVO: instrumentacion */
...
/* <-- NUEVO: getters/reset de la instrumentacion de arriba */
...
/* ################################################################
 * ##  NUEVO (EXPERIMENTAL, NO USADA POR DEFECTO): mutacion una  ##
 * ##  vez por cromosoma, para probar la hipotesis de que Pm     ##
 * ##  por-gen mutaba demasiado agresivo. Resultado medido: peor ##
 * ##  que mutate() (por gen) en k1 y mk03 -> se descarto, sigue ##
 * ##  disponible por si se quiere retomar el experimento.       ##
 * ################################################################ */
